...
export default function ReportsView({ invoices, expenses }: ReportsViewProps) {
    ...
    const { filteredInvoices, filteredExpenses, chartData } = useMemo(() => {
        ...
        fInvoices.forEach(inv => {
            ...
            if (dataMap.has(key)) {
                ...
                entry.Revenu += Number(inv.total_ttc);
    ...
    // --- KPIs ---
    const totalRevenue = filteredInvoices.filter(i => i.status === 'Paid').reduce((s, i) => s + Number(i.total_ttc), 0);
    const totalExpenses = filteredExpenses.reduce((s, e) => s + Number(e.amount), 0);
    const netProfit = totalRevenue - totalExpenses;
    const margin = totalRevenue > 0 ? ((netProfit / totalRevenue) * 100).toFixed(1) : 0;
    const pendingRevenue = filteredInvoices.filter(i => i.status !== 'Paid').reduce((s, i) => s + Number(i.total_ttc), 0);
    ...
    const COLORS = ['#3B82F6', '#EF4444', '#3B82F6', '#10B981', '#8B5CF6'];
        ...
        <div className="flex flex-col gap-8 pb-20 animate-fade-in">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`px-5 py-2 rounded-lg text-xs font-bold transition-all duration-300 ${range === tab.id
                                    ? 'bg-brand-gradient text-black shadow-lg scale-105'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                {[
                    ...
                    <div key={idx} className={`bg-white/[0.025] border border-white/[0.06] p-6 rounded-2xl relative overflow-hidden group hover:border-white/[0.14] hover:scale-[1.02] transition-all duration-200 cursor-default ${kpi.gradient ? 'bg-gradient-to-br from-primary/10 to-transparent' : ''}`}>
                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:scale-110 duration-500">
                            <span className="material-symbols-outlined text-7xl text-white">{kpi.icon}</span>
                        </div>
                        <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-2">{kpi.title}</p>
                        <h3 className={`text-xl lg:text-2xl font-bold tracking-tight leading-tight break-all ${kpi.color}`}>{formatMoney(kpi.amount)}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* BAR CHART */}
                <div className="lg:col-span-2 bg-white/[0.025] border border-white/[0.06] p-6 lg:p-8 rounded-2xl flex flex-col h-[400px]">
                    ...
                    <div className="flex-1 w-full min-h-0">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <Bar dataKey="Revenu" fill="#3B82F6" radius={[4, 4, 0, 0]} maxBarSize={40} />
                ...
                {/* PIE CHART */}
                <div className="bg-white/[0.025] border border-white/[0.06] p-6 lg:p-8 rounded-2xl flex flex-col h-[400px]">
                    <h3 className="text-white font-bold mb-6 flex items-center gap-3 text-sm uppercase tracking-wider">
            ...
            <div className="bg-white/[0.025] border border-white/[0.06] p-6 lg:p-8 rounded-2xl h-[350px] flex flex-col">
                <h3 className="text-white font-bold mb-6 flex items-center gap-3 text-sm uppercase tracking-wider">
